...
1. Open Windows Explorer and navigate to `C:\lab-files\CognitiveSearch`. Double-click on **`CognitiveSearch.sln`** to open the solution in Visual Studio. If you are prompted by Visual Studio to log in, use the Azure credentials you are using for this workshop.
   ...
   ![Windows Explorer window with `CognitiveSearch.sln` file highlighted.](imgs/windows-explorer-cognitive-search-sln.png 'Windows Explorer')
...
2. In the Solution Explorer on the left-hand side of Visual Studio, expand the **TweetGenerator** project, and then locate and open the `appsettings.json` file.
   ...
   ![In the Solution Explorer, `appsettings.json` is highlighted in the TweetGenerator project.](imgs/visual-studio-solution-explorer-appsettings.png 'Solution Explorer')
...
4. Return to the `appsettings.json` file in Visual Studio and paste the Primary Connection String into the value for the `COSMOS_DB_CONNECTION_STRING` setting. Your `appsettings.json` file should look similar to the following:
   ...
   ![Screenshot of the `appsettings.json` file.](imgs/appsettings-json-file.png 'appsettings.json')
...
6. Run the console app by selecting the button in the toolbar with the green arrow and text of **TweetGenerator**.
   ...
   ![The Run TweetGenerator button is displayed.](imgs/visual-studio-run-tweetgenerator.png 'Run TweetGenerator')
...
2. Select your **tech-immersion** Azure Cosmos DB account from the list of resources.
   ...
   ![The Azure Cosmos DB account resource is selected from the list of resources in the tech-immersion resource group.](imgs/tech-immersion-rg-cosmosdb.png 'Tech Immersion resource group')

3. Select **Data Explorer** from the toolbar on the overview blade

   ![Data Explorer is highlighted in the Cosmos DB toolbar in the Azure portal.](imgs/cosmos-db-toolbar-data-explorer.png 'Cosmos DB toolbar')

4. Under the `ContosoAuto` database, expand the **tweets** collection and then select **Documents**.

   ![Documents is selected and highlighted under the tweets collection in the `ContosoAuto` Cosmos DB database.](imgs/cosmos-db-tweets-collection-documents.png 'Documents')
...
5. In the documents pane, select any of the documents listed and inspect a tweet document. Documents are stored in JSON (JavaScript Object Notation) format in Cosmos.
   ...
   ![A tweet document is displayed.](imgs/cosmos-db-tweet-document.png 'Tweet document')
...
6. With a better understanding of the structure of the tweet documents stored in Cosmos DB, let's move on to creating a basic Cognitive Search pipeline. From your Cosmos DB blade in the Azure portal, select **Add Azure Search** from the left-hand menu, select your **tech-immersion** search service, and then select **Next: Connect to your data**.
   ...
   ![Add Azure Search is selected and highlighted on the left-hand menu and the tech-immersion search service is highlighted in the list of search services.](imgs/cosmos-db-add-azure-search-select-search-service.png 'Select a search service')
...
7. On the **Connect to your data** tab, enter the following:
   ...
   ![The Connect to your data tab is displayed, with the settings specified above entered into the form.](imgs/cosmos-db-add-azure-search-connect-to-data.png 'Connect to your data')
...
9. On the **Add cognitive search (Optional)** tab:
   ...
   ![The Add cognitive search tab is displayed, with the settings specified above entered into the form.](imgs/cosmos-db-add-azure-search-add-cognitive-search.png 'Add cognitive search')
...
11. On the **Customize target index** tab, do the following:
    ...
    ![The Customize target index tab is displayed, with the settings specified above entered into the form.](imgs/cosmos-db-add-azure-search-customize-target-index.png 'Customize target index')
...
13. On the **Create an indexer** tab, set the following:
    ...
    ![The Create an indexer tab is displayed, with the settings specified above entered into the form.](imgs/cosmos-db-add-azure-search-create-indexer.png 'Create an indexer')
...
15. Next, navigate to your Azure Search Service in the Azure portal by selecting it from the list of resources in the **tech-immersion** resource group.
    ...
    ![The tech-immersion Search Service is highlighted in the tech-immersion resource group.](imgs/tech-immersion-resource-group-search-service.png 'Resource group')

16. On the Azure Search service blade, select **Indexers**.

    ![Indexers is selected on the Azure Search service blade.](imgs/azure-search-indexers.png 'Indexers')
...
17. Your indexer was scheduled to run every 5 minutes, which is the minimum allowable time. If your indexer has a status of **No history**, you can bypass the scheduled time, and force the indexer to run, by selecting the indexer, and then selecting **Run** on the Indexer blade.
    ...
    ![The Run button is highlighted on the Indexer blade.](imgs/azure-search-indexers-run.png 'Indexer')

18. Once your Indexer has run, select Indexes, and select **tweet-index**.

    ![Azure Search Indexes are displayed, with tweet-index highlighted.](imgs/azure-search-indexes.png 'Indexes')

19. On the Search explorer tab, select **Search** and observe the results.

    ![The Search button is highlighted on the Azure Search Index blade.](imgs/azure-search-index-search.png 'Search')
...
2. To prepare the console application, you need to update the `appsettings.json` file for the project in information about your search service. Return to Visual Studio, and open the `appsettings.json` file located under the `PipelineEnhanceer` project.
   ...
   ![`appsettings.json` is highlighted under the PipelineEnhancer solution.](imgs/visual-studio-solution-explorer-pipeline-enhancer.png 'Solution explorer')
   ...
   ![The list of key values needed from `appsettings.json`.](imgs/appsettings-json-empty.png 'App settings')
...
4. On the overview blade of your search service, copy the **Url** field.
   ...
   ![Azure Search Service URL](imgs/azure-search-url.png 'Azure Search Service URL')
...
7. Return to your Search Service blade in the Azure portal, select **Keys** from the left-hand menu, and the select the **Copy** button for the **PRIMARY ADMIN KEY**.
   ...
   ![Azure search keys.](imgs/azure-search-keys.png 'Keys')
...
9. Return to the Azure portal, and select your **Cognitive Services** account from the list of resources under the **tech-immersion-XXXXX** resource group (where XXXXX is the unique identifier assigned to you for this workshop).
   ...
   ![The Cognitive Services resource is highlighted in the tech-immersion resource group.](imgs/tech-immersion-resource-group-cog-services.png 'Cognitive Services resource')
...
10. On the Cognitive Services blade, select **Properties** from the left-hand menu, and then copy the value for the **Resource ID** field by selecting the copy button to the right of the field.
    ...
    ![Cognitive Services properties blade.](imgs/cog-services-properties.png 'Properties')
...
12. Return to your Cognitive Services account in the Azure portal, and select **Keys** from the left-hand menu. On the Keys blade, copy the **Key 1** value by selecting the copy button to the right of the field.
    ...
    ![Cognitive Services keys.](imgs/cog-services-keys.png 'Keys')
...
14. The final settings you need to retrieve are those for your Azure Function App. In the Azure portal, navigate to your **tech-immersion-functions** Function App, and copy the **URL** on the Overview blade.
    ...
    ![The URL field is highlighted on the overview blade of the Function App.](imgs/function-app-url.png 'Function App')
...
16. Back in the Azure portal, select **Function app settings** on the Overview blade.
    ...
    ![Function app settings link.](imgs/function-app-settings.png 'Function app settings')

17. On the Function app settings tab, select the **Copy** button next to the **default** Host Key.

    ![The default host key copy button is highlighted.](imgs/function-app-default-host-key.png 'Function app settings default host key')
...
19. Save `appsettings.json`. The file should now look like the following.
    ...
    ![Search service values entered into the appsettings.json file.](imgs/pipeline-enhancer-app-settings-search-service.png 'App settings')
...
22. To add sentiment analysis to your pipeline you will run the `PipelineEnhancer` project within the **CognitiveSearch** solution in Visual Studio. To run the project, right-click the `PipelineEnhancer` project in Visual Studio and select **Set as StartUp Project**.
    ...
    ![In the Solution Explorer, PipelineEnhancer is highlighted, and the right-click contenxt menu is displayed. Set as StartUp Project is highlighted in the context menu.](imgs/visual-studio-pipeline-enhancer-startup-project.png 'Pipeline Enhancer')
...
23. Now, select the run button on the Visual Studio toolbar, which is the one with a green arrow followed by the text "PipelineEnhancer."
    ...
    ![The run button is displayed for PipelineEnhancer.](imgs/visual-studio-run-pipeline-enhancer.png 'Run button')

24. At the command prompt for the console app, enter **1** to incorporate the Sentiment cognitive skill to your pipeline.

    ![1 is entered at the prompt in the pipeline enhancer](imgs/pipeline-enhancer-1.png 'Pipeline Enhancer')
...
25. When the console app completes you will receive a message stating that the sentiment analysis skill was successfully added.
    ...
    ![A success message is displayed in the PipelineEnhancer console window.](imgs/pipeline-enhancer-sentiment-analysis-success.png 'Pipeline enhancer')
...
29. Return to the `PipelineEnhancer` console app and enter **2** at the prompt to add the new user-related fields to your pipeline.
    ...
    ![2 is entered at the prompt in the pipeline enhancer](imgs/pipeline-enhancer-2.png 'Pipeline Enhancer')
...
1. Since you will be using the Translator Text API, you need retrieve your API key from the Azure portal, so it can be used by the Azure Function. As you've done with previous resources, select the **tech-immersion-translator** resource from the **tech-immersion-XXXXX** resource group (where XXXXX is the unique identifier assigned to you for this workshop).
   ...
   ![The tech-immersion-translator resource is selected in the tech-immersion resource group.](imgs/tech-immersion-rg-translator.png 'Tech Immersion Resource Group - Translator')
...
2. Once on your Translator Text blade, select **Keys** from the left-hand menu, and then select the Copy button next to the value for **Key 1**.
   ...
   ![The copy button for Key 1 is highlighted on the Keys blade.](imgs/cognitive-services-translator-keys.png 'Keys')
...
3. Return to Visual Studio, and in the Solution Explorer on the right-hand side, expand the `TextTranslateFunction` project and then double-click `Function1.cs` to open the file.
   ...
   ![The `Function1.cs` file is highlighted in the Visual Studio Solution Explorer.](imgs/visual-studio-text-translate-function-function1.png 'Solution Explorer')
...
7. You are now ready to deploy the function into your Azure Function App. Right-click the `TextTranslateFunction` project, and select **Publish** from the context menu.
   ...
   ![Publish is highlighted in the context menu for the TextTranslateFunction project.](imgs/visual-studio-function-publish.png 'Publish')

8. On the **Pick a publish target** dialog, choose **Select Existing** and select **Publish**.

   ![Select Existing is highlighted and selected on the Pick a publish target dialog. The publish button is highlighted.](imgs/visual-studio-function-publish-target.png 'Pick a publish target')
...
9. On the **App service** dialog, select your Subscription and locate the **tech-immersion-XXXXX** resource group (where XXXXX is the unique identifier assigned to you for this workshop) in the list. Select your **tech-immersion-functions** Function App from this list of resources.
   ...
   ![The tech-immersion-functions Function App is highlighted in the search results.](imgs/visual-studio-function-publish-app-service.png 'App service')
...
10. Select **OK**. This will start the publish process. You will see an animated progress icon next to the Publish button while the deployment is in progress.
    ...
    ![The progress icon is highlighted on the Publish dialog.](imgs/visual-studio-function-publish-progress.png 'Publish')
...
11. When the publish is complete, you can open the **Output** window at the bottom left-hand corner of the Visual Studio window to observe the results. You should see messages that the **Publish Succeeded** and **Publish completed**.
    ...
    ![The publish succeeded and publish completed messages are highlighted in the Output window.](imgs/visual-studio-function-publish-output.png 'Output')
...
3. Return to the `PipelineEnhancer` console app window and enter **3** at the prompt. This step will incorporate the custom cognitive skill into your pipeline.
   ...
   ![3 is entered at the prompt in the pipeline enhancer](imgs/pipeline-enhancer-3.png 'Pipeline Enhancer')
...
4. When the console app completes, enter **X** at the prompt to close the application.
   ...
   ![X is entered at the prompt in the pipeline enhancer](imgs/pipeline-enhancer-x.png 'Pipeline Enhancer')
...
1. In the Azure portal, navigate to your **tech-immersion** Search service, select the **Search explorer** in the toolbar on the overview blade.
   ...
   ![Azure Search Service blade is displayed, with Search explorer highlighted.](imgs/azure-search-explorer.png 'Search explorer')
...
3. You can now play around with the search functionality. Enter a few queries that simulate what an application user may enter. These queries tend to resemble natural language, so we will start there. In the search field, enter "language is es", and observe the results. This will return one of the records in the search results where the language was detected as Spanish.
   ...
   ![Query for records where language is es.](imgs/azure-search-query-language-is-es.png 'Search query')
...
4. Try another search, such as "cold battery" and observe the results.
   ...
   ![Query for records containing "cold" and "battery"](imgs/azure-search-query-cold-battery.png 'Search query')
...
5. Now, let's try a slightly more advanced search. We want to look for only records that mention a "corvette", and we only want to retrieve the `text`, `userLocation`, and `sentiment` fields in our results. Paste the following into the search box, and select **Search**:
   ...
   ![Query for records containing corvette, and returning only the text, userLocation, and sentiment.](imgs/azure-search-query-corvette.png 'Search query')
...
6. You can take that query even further by adding in the `$filter` parameter. Use the `$filter` parameter when you want to specify precise criteria rather than free text search. This example searches for sentiment lass than 0.25, so we can only return tweets with negative sentiment.
   ...
   ![Using the $filter parameter to further refine search results.](imgs/azure-search-query-filter.png 'Filtered search query')
...
7. The final query we will run adds the `$orderBy` parameter, which allow you to specify the sort order of your results. In this case, let's search for records where the sentiment is the highest, filtering for records where the sentiment is greater than 0.9, and ordering the results in descending order.
   ...
   ![Using the orderBy filter in a search query](imgs/azure-search-query-orderby.png 'Ordered search query')