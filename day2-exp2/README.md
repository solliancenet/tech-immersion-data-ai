# Day 2, Experience 2 - Yield quick insights from unstructured data with Knowledge Mining, Cognitive Services, and Power BI
...
- [Day 2, Experience 2 - Yield quick insights from unstructured data with Knowledge Mining, Cognitive Services, and Power BI](#day-2-experience-2---yield-quick-insights-from-unstructured-data-with-knowledge-mining-cognitive-services-and-power-bi)
  ...
  - [Task 7: Build Power BI dashboard](#task-7-build-power-bi-dashboard)
...
1. Open Windows Explorer and navigate to `C:\lab-files\CognitiveSearch`. Double-click on **CognitiveSearch.sln** to open the solution in Visual Studio. If you are prompted by Visual Studio to log in, use the Azure credentials you are using for this workshop.
...
3. Next, you need to retrieve your Cosmos DB connection string from the [Azure portal](https://portal.azure.com). This will be used to enable the TweetGenerator to write data into your `tweets` collection.

   - In the Azure portal, navigate to your Cosmos DB instance by selecting it from the list of resources under the **tech-immersion** resource group.
   - On your Cosmos DB blade, select **Keys** from the left-hand menu.
...
4. Return to the `appsettings.json` file in Visual Studio and paste the Primary Connection String into the value for the `COSMOS_DB_CONNECTION_STRING` setting. Your `appsettings.json` file should look similar to the following:

   ![Screenshot of the appsettings.json file.](media/appsettings-json-file.png 'appsettings.json')

5. Save `appsettings.json`.

6. Run the console app by selecting the button in the toolbar with the green arrow and text of **TweetGenerator**.
...
1. In the [Azure portal](https://portal.azure.com), select **Resource groups** from the left-hand menu, and then select the resource group named **tech-immersion**.

   ![The tech-immersion resource group is selected.](media/tech-immersion-rg.png 'Resource groups')

2. Select your **tech-immersion** Azure Cosmos DB account from the list of resources.
...
3. Select **Data Explorer** from the toolbar on the overview blade
...
4. Under the ContosoAuto database, expand the **tweets** collection and then select **Documents**.
...
5. In the documents pane, select any of the documents listed and inspect a tweet document. Documents are stored in JSON (JavaScript Object Notation) format in Cosmos.
...
7. On the **Connect to your data** tab, enter the following:
...
10. Select **Next: Customize target index**.

11. On the **Customize target index** tab, do the following:
...
12. Select **Next: Create an indexer**.

13. On the **Create an indexer** tab, set the following:
...
15. Next, navigate to your Azure Search Service in the Azure portal by selecting it from the list of resources in the **tech-immersion** resource group.
...
16. On the Azure Search service blade, select **Indexers**.

    ![Indexers is selected on the Azure Search service blade.](media/azure-search-indexers.png 'Indexers')

17. Your indexer was scheduled to run every 5 minutes, which is the minimum allowable time. If your indexer has a status of **No history**, you can bypass the scheduled time, and force the indexer to run, by selecting the indexer, and then selecting **Run** on the Indexer blade.

    ![The Run button is highlighted on the Indexer blade.](media/azure-search-indexers-run.png 'Indexer')

18. Once your Indexer has run, select Indexes, and select **tweet-index**.

    ![Azure Search Indexes are displayed, with tweet-index highlighted.](media/azure-search-indexes.png 'Indexes')

19. On the Search explorer tab, select **Search** and observe the results.

    ![The Search button is highlighted on the Azure Search Index blade.](media/azure-search-index-search.png 'Search')

20. Looking at the items in the search results, you will see that they are resemble the following:
...
1. Since you will be using the Translator Text API, you need retrieve your API key from the Azure portal, so it can be used by the Azure Function. As you've done with previous resources, select the **tech-immersion-translator** resource from the **tech-immersion** resource group.
...
2. Once on your Translator Text blade, select **Keys** from the left-hand menu, and then select the Copy button next to the value for **Key 1**.
...
9. On the **App service** dialog, select your Subscription and locate the **tech-immersion** resource group in the list. Select your **tech-immersion-functions** Function App from this list of resources.
...
1. In the Azure portal, navigate to your **tech-immersion** Search service, select the **Search explorer** in the toolbar on the overview blade.
...
3. You can now play around with the search functionality, by entering a few queries that would simulate what an application user may enter. These queries tend to resemble natural language, so we will start there. In the search field, enter "language is es", and observe the results.

   ![Query for records where language is es.](media/azure-search-query-language-is-es.png 'Search query')

   > Notice that each record returned contains a `@search.score` field with a numeric value. This value indicates the confidence of the match with the search query. The higher the value, the more likely it is to be a match for the query. If you scroll down past the records where `"language"="es"`, you will see the search score values drop, as those records don't match the query.
...
5. Now, let's try a slightly more advanced search. We want to look for only records that mention a "corvette", and we only want to retrieve the `text`, `useLocation`, and `sentiment` fields in our results. Paste the following into the search box, and select **Search**:
...
6. You can take that query even further by adding in the `$filter` parameter. Use the `$filter` parameter when you want to specify precise criteria rather than free text search. This example searches for sentiment lass than 0.25, so we can only return tweets with negative sentiment.
...
## Task 7: Build Power BI dashboard

In this task, you will build a Power BI dashboard summarizing tweet sentiment and top entities referenced.

## Additional resources

To continue learning and expand your understanding of Knowledge Mining with Cognitive Search, check out the following resources:

- [Learn Cognitive Search](https://azure.github.io/LearnAI-Cognitive-Search/)
- [Introduction to Cognitive Search](https://docs.microsoft.com/en-us/azure/search/cognitive-search-concept-intro)
